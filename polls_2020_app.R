...
plot_spread = function(data){
    ggplot(data, aes(x=state_gen, y = to_spread(est), ymin = to_spread(lb), ymax = to_spread(ub))) +
        ...
        geom_point(aes(text = sprintf("Spread estimate: %.2f <br>State: %s", to_spread(est), state)), 
                   size = 1) +
        # geom_point(aes(y = res), color = "blue", shape = 17, size = 2) +
                    ...
                    tabPanel("Forecasts",
                             sidebarLayout(
                                 sidebarPanel(
                                     ...
                                     helpText("Updated July 13, 2020.")
                                 ),
                                 mainPanel(plotlyOutput("plot2"),
                                 
                                 br(),
                                 
                                 p(paste0("Biden Electoral College Votes: ", dem_votes)),
                                 
                                 p(paste0("Trump Electoral College Votes: ", rep_votes)),
                                 
                                 p(paste("Current toss-up states: ", paste(tossup, collapse = ", ")))
